...
demo= xlsread('trackon_fsv7'); %covariates - age, gender, site, TIV
covars = demo(:,2:5);

% Load matrices
[ndata, s_ind, alldata] = xlsread('trackon_fsv7','A:A');
s_ind(1,:)=[];
%s_ind = s_ind';
cd trackon/
...
cont_phd_dm = [cat(1,ones(m.cont,1),zeros(m.phd,1)) cat(1,zeros(m.cont,1),ones(m.phd,1)) phd_cont_dm([m.phd + 1:end 1:m.phd],3:6)];
